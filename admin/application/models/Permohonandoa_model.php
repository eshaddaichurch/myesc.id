...
class Permohonandoa_model extends CI_Model
{
    ...
    public function update($data, $idpermohonan)
    {
        try {
            ...
            // Buat Notifikasi 
            $query = $this->db->query("SELECT idjemaat FROM carepermohonandoa WHERE idpermohonan = ?", $idpermohonan);
            if ($query->num_rows() === 0) {
                throw new Exception("Data permohonan doa tidak ditemukan untuk ID: $idpermohonan");
            }
            ...
            $notifikasi = array(
                ...
                'deskripsi' => 'Care telah mengkonfirmasi permohonan doa anda.',
                'linknotifikasi' => 'permohonandoa/detail/' . $this->encrypt->encode($idpermohonan),
